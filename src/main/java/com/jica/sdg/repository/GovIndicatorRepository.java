...
public interface GovIndicatorRepository extends CrudRepository<GovIndicator, Integer> {
	...
	@Query(value = "select a.* from gov_indicator a left join gov_activity b on a.id_activity=b.id where b.id_role = :id_role",nativeQuery = true)
	public List<GovIndicator> findAllByRole(@Param("id_role") Integer id_role);
	
	@Query(value = "select a.id, a.id_gov_indicator,a.nm_indicator,b.id_unit,b.nm_unit,a.nm_indicator_eng, a.internal_code from gov_indicator a left join ref_unit b on a.unit=b.id_unit where a.id_program = :id_program and a.id_activity = :id_activity",nativeQuery = true)
