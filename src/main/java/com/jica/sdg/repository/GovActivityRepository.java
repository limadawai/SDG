...
public interface GovActivityRepository extends CrudRepository<GovActivity, Integer> {
	@Query(value = "select * from gov_activity where id_program = :id_program and id_role = :id_role",nativeQuery = true)
	public List<GovActivity> findAllGovActivity(@Param("id_program") Integer id_program, @Param("id_role") Integer id_role);
	
	@Query(value = "select a.* from gov_activity a left join ref_role b on a.id_role=b.id_role where a.id_program = :id_program and b.id_prov = :id_prov",nativeQuery = true)
